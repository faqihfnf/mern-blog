    ...
    const fetchPosts = async () => {
      try {
        ...
        const res = await fetch(
          `/api/post/getposts?userId=${currentUser._id}&page=${currentPage}&limit=${postsPerPage}`
        );
        ...
        <div className="mb-4">
          <Link to={"/create-post"}>
            <Button type="button" gradientDuoTone="purpleToPink" className="">
              ...
              <span className="font-poppins text-md">Create Post</span>
            </Button>
          </Link>
        </div>
                ...
                <Table.Body className="divide-y">
                  {userPosts.map((post) => (
                    <Table.Row
                      ...
                      <Table.Cell>
                        <Link
                          className="font-medium line-clamp-2 text-slate-900 dark:text-white"
                          to={`/post/${post.slug}`}>
                          {post.title}
                        </Link>
                      </Table.Cell>
